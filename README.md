...
service's own business API in the same process — **never enable `admin` on `Zanix.start()` in that
same process**; a runtime guard throws if both try to register the admin routes at once.
...
`TriggersAdminClient`/`TemplatesAdminClient` and `ZanixAdmin`, and how this service's own admin API
relates to the centralized `ZanixAdmin` orchestrator.
...
- [`docs/admin-architecture.md`](./docs/admin-architecture.md) — this service's admin API vs. the
  centralized `ZanixAdmin` orchestrator.
